/* ════════════════════════════════════════════════════
   LinkPortal Firefox Extension — background.js  v1.10.9
   Basic Auth · Periodic Sync · 30-day Logout · Bookmark mirror
   Firefox: uses the promise-based browser.* API (aliased to chrome.*)
   ════════════════════════════════════════════════════ */

// Firefox provides a promise-based browser.* API — alias it to chrome.* so `await chrome.*` works
...
// Safari: OffscreenCanvas available in service workers from Safari 16.4+
async function restoreIcon() {
  try {
    ...
    if (typeof OffscreenCanvas !== 'undefined') {
      ...
      // Fallback: set path directly (Safari <16.4 in service worker context)
      // The icon will be set by popup.js on next open
      console.info('[LinkPortal] OffscreenCanvas not available, icon will be set on next popup open');
    }
  } catch (e) {
